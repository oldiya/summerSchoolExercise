...
format:
   titlepage-pdf:
    ...
    toc: true
    titlepage: "bg-image"
    titlepage-bg-image: "images/cartoon1.png"
...
In this session, we will cover two examples of how to create two models based on observed data. We will use two approaches Generalized Linear Models (GLMs) and Boosted Regression trees (BRTs).

Please take all the results with a grain of salt, we are making very generalized statements from a limited data set and we are not getting into the details of how each of the models should be assessed, the goal here is that you learn how observed data can be used to create a models that helps you to better understand the data and relationships.

In this document we describe all the exercises, but your group has been assigned to only one which include to using one specific modelling approach. Each group has two questions to be answered (one question per subgroup inside each group). Please contact your group coach if you are unsure what section you should be focusing on. The table below summarizes and links to the different parts of this document:

| Group | Sub | Question number | Question                                                                                                    | Approach | Go to section |
|-----------|-----------|-----------|----------------|-----------|-----------|
| 1     | 3   | 1               | Does a more diverse forest in structure and composition have more Bryophites species?                       | GLM      |               |
| 1     | 4   | 2               | Is the number of Bryophites species affected by forest management type and the forest structural diversity? | GLM      |               |
| 2     | 3   | 3               | Does a more diverse forest in structure and composition have more bird species?                             | GLM      |               |
| 2     | 4   | 4               | Is the number of bird species affected by forest management type and the forest structural diversity?       | GLM      |               |
| 3     | 3   | 5               | Is the presence of the Great spotted woodpecker affected by forest density?                                 | BRT      |               |
| 3     | 4   | 6               | Is the presence of the Great spotted woodpecker affected by forest diversity?                               | BRT      |               |
| 4     | 3   | 7               | Is the presence of the Eurasian treecreeper affected by forest density?                                     | BRT      |               |
| 4     | 4   | 8               | Is the presence of the Eurasian treecreeper affected by forest management?                                  | BRT      |               |

# Data

We will work with one dataset derived from the inventory developed to assess forest structure and dead wood properties of six representative forest areas in the Czech Republic [@hosek]. This dataset collects observations of presence /absence of certain species of biodiversity importance across 99 plots.

The data was collected from square sampling plots (2500 m2 each) located in six forested regions in the Czech Republic. These regions are representative of the main bio-regions and elevation range of forests, with consideration of their importance in terms of territorial representation, forestry, and ecology. The inventory sampled for biodiversity variables such as the presence / absence of different species of birds, tracheophyta, bryophytes, fungi, lichens and beetles.

The dataset has been generously provided by XXXXXX to be used in the context of this exercise. It should not be used for other purposes or be further distributed. If you are interested in using this data or learning more about it please contact XXXX in here: XXXX.

The data you can download for this exercises is an aggregated summary from the original data, some aspects have been modified from the observed data, so the results you will obtain will not completely match the observed reality. This data is very similar in structure and observed variables as the one you have collected in this summer school. The idea is that you move from data collection, analysis to this part where you use the data to create a model.
...
-   `GiniDBH`: Gini index calculated to assess the forest structural diversity in diamter sizes, higher values indicate more structural heterogeneity; lower values indicate more homogeneous stands
...
We propose to use a generalized linear model (GLM) to understand the abundance of different bryophites species at the 99 plots. Count data often conform to a Poisson distribution, in this case we have a count of the number of species recorded at each plot.
...
Our recomendation is that you follow this folder structure and you put the data in the folder data of the project and the .rproj into the main folder. If you decide to organized things in a different way then you will have to change the path to the code provided to load the data in the section "Explore the data"

PHOTO OF THE FOLDER STRUCTURE HERE

You can download the whole project with this document, the rproj and the correct folder structure in here \[2\] but this has to be done using git.
...
You should then explore the data and understand its structure. You should pay attention to the meaning of each variable and the type of data structure. In this dataset each row is one plot (see `PlotID`), and each column is one variable.
...
-   What am I trying to understand with this model? (aka your question)
...
Here you can see that the coefficient table produced by a GLM is very similar to a linear model. The intercept tells us the estimated value of the response variable when the continuous explanatory variables (here just Gini index) has a value of 0. We then also have coefficients describing the slope of the relationship with our continuous explanatory variables. We can see here that the number of Bryophytes species appears to show a positive relationship with Gini index, which means that increasing structural diversity in trees diameter sizes has a positive relationship with the number of Bryophytes species in the plot.

We are also interested in understanding the relationship with the number of Bryophytes species and the trees species diversity, we can now try to add this variable into the model and see if it help us to understand things. You can do that by adding the variable `ShannonIndexTreeSpp` into the model
