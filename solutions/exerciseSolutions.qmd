...
```{r not to show, echo=T, include=FALSE}
...
#     classoption: ["oneside", "open=any",  DIV=16, fontsize=11pt]
...
This document contains the instructions and solutions of the modeling group assignments. First, you will find the section for the process model approach (@sec-pb) and after that the section for the empirical modelling (@sec-em). This document is very long because it describes all the exercises and solutions step by step.
...
|--------|------------|-------------------|---------------|----------|
| 1     | A                 | How are biodiversity indices changing in time and across the simulated scenario(s) on Plot 1?              | iLand    | @sec-A_1 |
| 1     | B                 | How the species distribution and total living biomass C content changing in time on Plot 1?                | iLand    | @sec-B_1 |
| 1     | C                 | Does a more diverse forest in structure and composition have more Bryophyte species?                       | GLM      | @sec-C_1 |
| 1     | D                 | Is the number of Bryophyte species affected by forest management type and the forest structural diversity? | GLM      | @sec-D_1 |
| 2     | A                 | How are biodiversity indices changing in time and across the simulated scenario(s) on Plot 2?              | iLand    | @sec-A_2 |
| 2     | B                 | How the species distribution and total living biomass C content changing in time on Plot 2?                | iLand    | @sec-B_2 |
...
| 3     | A                 | How are biodiversity indices changing in time and across the simulated scenario(s) on Plot 3?              | iLand    | @sec-A_3 |
| 3     | B                 | How the species distribution and total living biomass C content changing in time on Plot 3?                | iLand    | @sec-B_3 |
...
| 4     | A                 | How are biodiversity indices changing in time and across the simulated scenario(s) on Plot 4?              | iLand    | @sec-A_4 |
| 4     | B                 | How the species distribution and total living biomass C content changing in time on Plot 4?                | iLand    | @sec-B_4 |
...
## Data and model {#sec-policy}

We provide all data, model and scripts for the exercises. However if you will use the model iLand for your research in the future please get in contact with the model developers at the Technical University Münich: Werner Rammer (werner.rammer\@tum.de) and Rupert Seidl (rupert.seidl\@tum.de). Regarding the biodiversity data used in the empirical modeling part please contact Jeňýk Hofmeister at the Czech University of Life Sciences Prague (jenyk.hofmeister\@email.cz) if you are interested in using this data further.

### The project folder {#sec-projFolder}
...
<https://drive.google.com/drive/folders/1IXfJNIXw32lp1q4AmEZzt0nqgE3HDRIx?usp=drive_link>.

In this project you will find the following folders and files:

-   `summerSchoolExercise.Rproj`: this is the RStudio project file that you should open. This file contains various project options and is used as a shortcut for opening the project directly from the file system.
-   `code`: this folder is where you should save the R scripts .
-   `data`: this folder contains the data used in the empirical modeling.
-   `instructions`: this folder is where you can find the pdf and qmd (source code document used to compile the pdf) with the step by step instructions for the exercises without the solutions; this folder also contains all the extensions and images needed to compile the document.
-   `model`: this folder is where you will find the folder `iLand_simulations` with all the files needed to run the process based exercise.
-   `solutions`: this folder is where you can find this document in pdf and in the qmd; this folder also contains all the extensions and images needed to compile the document
...
A)  How are biodiversity indices changing in time and across the simulated scenario(s)?

B)  How are the species distribution and total living biomass C content changing in time? Compare 0 year and 100 year status in the reference case and in the case of your scenario(s)

### Getting started {#sec-gs}

Go to the `model/iLand_simulations` folder inside the project folder. Here you will find several folders that are the part of the model, others are containing input data for the model and one folder is dedicated for the outputs. Additionally, you will find `iland.exe` and `.xml` files. The `.xml` file is called `project file`. This file is driving the settings of simulations, such as input/output file names, output variables and many other settings. This file is what you need to run in the model.
...
More details about the project file structure and meaning of each record can be found here: [https://iland-model.org/project+file](https://iland-model.org/project+file)

We prepared one project file for each group (for each simulation plot). You can right away run it, and then copy-paste and modify it to specify your own scenarios for alternative simulations. You can open a project file in any text editor, but we recommend to use Notepad++ for this ([https://notepad-plus-plus.org/downloads/](https://notepad-plus-plus.org/downloads/)).

### Explore input files {#sec-in}
...
-   Species parameter file: database/species_param_europe.sqlite Containing the parameters for each species that are driving growth, mortality, establishment and other processes. [https://iland-model.org/species+parameter](https://iland-model.org/species+parameter)
...
-   Climate file: `database/E-OBSv27_Roznik_46.05_14.45_1961-1990.sqlite`
    Daily climate data for the area based on the E-OBS dataset. [https://iland-model.org/ClimateData](https://iland-model.org/ClimateData) [https://surfobs.climate.copernicus.eu/dataaccess/access_eobs.php#datafiles](https://surfobs.climate.copernicus.eu/dataaccess/access_eobs.php#dat) We are using the same climate input for all plots. The E-OBS data is representative for a 0.1x0.1 degree resolution gridbox
...
For the sake of simplicity we assume now no-management scenario for all simulations.

NOTE: in the graphical interface you also will not see trees smaller than 4m, because they are not simulated as individuals rather in cohorts. But as regeneration will work, after a while as they grow above 4m, they become individual trees and we can see them also visually and they have own properties (dbh, height, age,...[https://iland-model.org/tree+variables](https://iland-model.org/tree+variables). More details on saplings: [https://iland-model.org/sapling+growth+and+competition](https://iland-model.org/sapling+growth+and+competition)

Setting the output tables: In the output section you can set which tables you want to enable. Here you can find more info on the structure and variables of each table: [https://iland-model.org/Outputs](https://iland-model.org/Outputs)
...
-   Wait until the simulation area is shown in the middle.
...
### Working with the output {#sec-out}

The output database has a `.sqlite` extension. You can explore outputs:

-   Open and browse it in the `DB Browser` software that was on the software list. ([https://sqlitebrowser.org/](ttps://sqlitebrowser.org/))
...
# you should change the name of the file according to your group, e.g. 
# Group 1 is Output_plot1.sqlite
# Group 2 is Output_plot2.sqlite
# Group 3 is Output_plot3.sqlite
# Group 4 is Output_plot4.sqlite
file <-
...
### Settings to consider for the assessment of model results {#sec-se}

#### No management

We simulate stand development without management activities, which most likely generates accumulation of carbon and volume on the stands in this 100 years period. Generally iLand has a very sophisticated so called "agent based management engine" functionality for large landscapes, but also simple management activities are possible for stand-scale simulations. However, for the sake of simplicity in these examples we do not simulate any management activities.

#### Initialization of trees

iLand considers trees higher than 4m as individuals. All trees smaller than this size are simulated as cohorts. We initialize individual trees in year=0, but no regeneration layer, however the process of regeneration is turned on, consequently trees are producing seeds and after the establishment they start to grow. It takes some 15-20-25 years to be able to see this regeneration effect on the outputs which we will study (that are based on individual trees). You can find more information here: [https://iland-model.org/regeneration?highlight=regeneration](https://iland-model.org/regeneration?highlight=regeneration)

#### Closed system

We assume no external seed input to our stand. This is highly unrealistic, as seeds can enter to our 1 ha area from the surrounding forests and bring species which are not there as individual trees in the beginning. Additionally, we are using the so-called torus mode so everything that escapes on one side of the pixel, enters on the other side. In iLand there is an option to set background seed availability to different species assuming probabilities for establishment in the project file, which we turned off for now. In this way new species cannot appear on the stand during the simulation time.

Practically the probabilities can be set based on the knowledge about the potential natural vegetation in the region, or based on the knowledge of the surrounding area. You can try to set external seeds if you wish in the project file turning the *externalSeedEnabled* from false to true. And set some probabilities (really small numbers) at *externalSeedBackgroundInput* a bit below down:

```{r ex10b, eval=FALSE, message = FALSE, warning=FALSE}

<externalSeedEnabled>true</externalSeedEnabled>        
...

<externalSeedBackgroundInput>fasy 0.0004 abal 0.0004 qupe 0.0002 tico 0.00002
cabe 0.00001 potr 0.00001 frex 0.00001 pisy 0.00001 bepe 0.00001 soau 0.00001 
piab 0.00001 acps 0.00001</externalSeedBackgroundInput>		  
     
```

More information on external seeds are [here: https://iland-model.org/external+seeds?highlight=seed](https://iland-model.org/external+seeds?highlight=seed)

### Group 1 - Question A {#sec-A_1}

-   How are biodiversity indices changing in time and across the simulated scenario(s) on Plot 1?

Run the model as it is described in the previous chapters @sec-run, @sec-as and @sec-out to have at least 2 simulations completed: one with reference conditions and one scenario with some changes in the environment. Check if you have the output files in the output folder: *iLand_simulations/output/*. We show all of the solutions on the example of one reference and one alternative scenario output file. Open the *summerSchoolExercise.Rproj* project in R studio and start working there.

#### Read the tree output table {#sec-A_1.1}
...
Merge the data from the two files together and make a column caller run which tells which comes from which simulation, and study the table. The column "run" will support the plotting where we can use facets in ggplot.
...
#### Visualize trees in year 0 and year 100  {#sec-A_1.2}
...
#### Visualize some changes in time  {#sec-A_1.3}
...
We can see that there is an initial increase in mean dbh, then a drop after 20/25 years. This is due to the growing regeneration layer that is produced the seeds of the existing trees. In the initial year we do not have regeneration layer in the simulations (it is possible to put there, but we do not have it now). And until the small trees grow up to higher than 4 m they are not appearing in the individual *tree* output as they are handled in cohorts.

#### Species diversity {#sec-A_1.4}
...
Where ${p~i~}$ is the relative abundance of the species *i*, calculated as the ratio of the abundance of the species i and the abundance of all species. Shannon-index is 0 when we have only one species at the stand. Here we calculate the abundance based on the number of trees, but can be calculate based on the volume to give more weight to larger trees. We add N as the number of records in the summarizing process below, but we can sum up the volume_m3 column and prepare the table for the speciesdiv function based on the volume (or any other variable, e.g. basal area).
...
```{r ex18, eval=TRUE, message = FALSE, warning=FALSE}
div <- rbind(div1, div2)
...
We see that the values of Shannon index are first stable and then tend to decrease, while the reduction under the 4deg_20percdrier is more substantial than under the reference scenario.

#### Spatial diversity {#sec-A_1.5}

We can calculate some additional biodiversity indices targeting spatial diversity using the *treespat* package developed by Francesco Chianucci (fchianucci\@gmail.com). You can find more information about this R package following this \[link\](https://www.biorxiv.org/content/10.1101/2023.05.22.541683v1.full).

You can install the package using devtools:

```{r ex19, eval=TRUE, message = FALSE, warning=FALSE}
...
```{r ex20, eval=TRUE, message = FALSE, warning=FALSE}
...
#### Read the landscape output table {#sec-B_1.1}

Read in the *landscape* output table from two outputs you have for your plot:

```{r ex22, eval=TRUE, message = FALSE, warning=FALSE}
path1 <- "model/iLand_simulations/output/Output_plot1.sqlite"
path2 <-
    "model/iLand_simulations/output/Output_plot1_4deg_20percdrier.sqlite"
...
```{r ex23, eval=TRUE, message = FALSE, warning=FALSE}
...
#### Visualize changes in time {#sec-B_1.2}
...
```{r ex24, eval=TRUE, message = FALSE, warning=FALSE}
...
Note that we do not have management intervention, and in the model trees shorter than 4m are not included in these outputs. However as they grow taller than 4m, model starts to handle them as individual trees and including in these outputs which we are looking now.
...
```{r ex26, eval=TRUE, message = FALSE, warning=FALSE}
species.livingC0 <-
    data.frame(
        landscape |> dplyr::filter(year == 0) |> 
...
#### Assess the species proportions {#sec-B_1.4}
...
```{r ex27, eval=TRUE, message = FALSE, warning=FALSE}
...
```{r ex28, eval=TRUE, message = FALSE, warning=FALSE}
LC <- rbind(LC0, LC100)
...
### Group 2 - Question A {#sec-A_2}

-   How are biodiversity indices changing in time and across the simulated scenario(s) on Plot 2?

Run the model as it is described in the previous chapters @sec-run, @sec-as and @sec-out to have at least 2 simulations completed: one with reference conditions and one scenario with some changes in the environment. Check if you have the output files in the output folder: *iLand_simulations/output/*. We show all of the solutions on the example of one reference and one alternative scenario output file. Open the *summerSchoolExercise.Rproj* project in R studio and start working there.

#### Read the tree output table {#sec-A_2.1}

Read in the *tree* output table from two outputs you have for your plot:

```{r ex29, eval=TRUE, message = FALSE, warning=FALSE}
...
```{r ex30, eval=TRUE, message = FALSE, warning=FALSE}
...
#### Visualize trees in year 0 and year 100  {#sec-A_2.2}

To explore a bit the simulation, plot the tree locations at the beginning and the end of the simulation For plotting we use the colors for the species, and the size of a circles to show dbh differences. Here we divided dbh with the value 20 just for visualization, not to have too huge circles shading each others completely, but still see the trees.

```{r ex31, eval=TRUE, message = FALSE, warning=FALSE}
tree0 <- tree |> dplyr::filter(year == 0)
g1 <-
    ggplot2::ggplot(tree0, ggplot2::aes(x = x, y = y, color = species)) +
    ggplot2::geom_point(size = tree0$dbh / 20, alpha = 0.7) +
...
#### Visualize some changes in time  {#sec-A_2.3}

Visualize some changes in time, for example the mean diameter of the trees

```{r ex32, eval=TRUE, message = FALSE, warning=FALSE}
sum.table <- tree |> dplyr::group_by(year, run) |>
    dplyr::summarise(
        N = dplyr::n(),
        MD = mean(dbh, na.rm = TRUE),
...
```{r ex33, eval=TRUE, message = FALSE, warning=FALSE}
...
```{r ex34, eval=TRUE, message = FALSE, warning=FALSE}
...
```{r ex35, eval=TRUE, message = FALSE, warning=FALSE}
tlong2 <- tree2 |>
    dplyr::group_by(year, species) |> dplyr::summarise(N = dplyr::n()) |>
    tidyr::pivot_wider(
        id_cols = 'year',
...
```{r ex36, eval=TRUE, message = FALSE, warning=FALSE}
...
We see that the values of Shannon index are first increasing and then tend to decrease, while the reduction under the 4deg_20percdrier is starting later than under the reference scenario.

#### Spatial diversity {#sec-A_2.5}

We can calculate some additional biodiversity indices targeting spatial diversity using the *treespat* package developed by Francesco Chianucci (fchianucci\@gmail.com). You can find a short description of the package here: *"documents/treespat_package.pdf"*

You can install the package using devtools:

```{r ex37, eval=TRUE, message = FALSE, warning=FALSE}
devtools::install_gitlab('fchianucci/treespat')
```
...
```{r ex38, eval=TRUE, message = FALSE, warning=FALSE}
library(treespat)
...
```{r ex39, eval=TRUE, message = FALSE, warning=FALSE}
...
The diameter differentiation increases with the increasing average difference in diameter between neighboring trees. The value 0 indicates that neighboring trees have equal diameters. The development shows that the index values decrease during the first circa 10 years, after which it steeply increases due to the occurrence of ingrowth and then the values are stabilized. The increase is not that steep for the climate change sceanario that is in accordance with the mean dbh graph. The values for the reference climate scenario are higher than for the climate change scenario indicating slower growth under drier and warmer conditions and/or the differences in tree species composition (proportion of tree species). The mingling index is studying the neighboring trees regarding their species. In both development there is a decrease in the index after the occurrence of regeneration. Using the climate change scenario the index shows steeper decrease and ending up at lower values after 100 years.

### Group 2 - Question B {#sec-B_2}
...
#### Read the landscape output table {#sec-B_2.1}

Read in the *landscape* output table from two outputs you have for your plot:

```{r ex40, eval=TRUE, message = FALSE, warning=FALSE}
path1 <- "model/iLand_simulations/output/Output_plot2.sqlite"
path2 <-
    "model/iLand_simulations/output/Output_plot2_4deg_20percdrier.sqlite"
...
```{r ex41, eval=TRUE, message = FALSE, warning=FALSE}
...
#### Visualize changes in time {#sec-B_2.2}

We can see that the output is given for each year and each species in our 1ha area. Plot the living carbon (total_carbon_kg) in time, coloring by species. We give a unified species coloring in the beginning. Plot number of trees and mean diameter. Carbon and number of trees can be plotted in an additive way, but for mean dbh we do line plot per species.

```{r ex42, eval=TRUE, message = FALSE, warning=FALSE}
...
# Plot the living carbon content, to have tonnes/ha, we divide total_carbon_kg by 1000.
g1 <-
    ggplot2::ggplot(landscape, ggplot2::aes(year, total_carbon_kg / 1000 , fill = factor(species))) +
    ggplot2::geom_area() +
    ggplot2::scale_fill_manual(values = cols.all, 
                               guide = ggplot2::guide_legend(reverse = TRUE)) +
    ggplot2::facet_wrap( ~ run, nrow = 1) +
...
#### Assess the stored carbon amount {#sec-B_2.3}

Calculate the stored C amount in the initial year (year==0) and the last year (year==100)

```{r ex43, eval=TRUE, message = FALSE, warning=FALSE}
livingC0 <- data.frame(
    landscape |> dplyr::filter(year == 0) |>
        dplyr::group_by(run) |>
        dplyr::summarize(sum.livingC = sum(total_carbon_kg / 1000))
...
```{r ex44, eval=TRUE, message = FALSE, warning=FALSE}
species.livingC0 <-
    data.frame(
        landscape |> dplyr::filter(year == 0) |>
            dplyr::group_by(run, species) |>
...
#### Assess the species proportions {#sec-B_2.4}

Calculate the species proportions based on the stored C amount in the initial year (year==0) and the last year (year==100) For this we need the total C amount and the species-specific C amount.

```{r ex45, eval=TRUE, message = FALSE, warning=FALSE}
LC0 <- dplyr::left_join(species.livingC0, livingC0, by = "run")
LC0 <-
    data.frame(LC0 |> dplyr::mutate(spec.prop = livingC / sum.livingC, year =
                                        0))
...
```{r ex46, eval=TRUE, message = FALSE, warning=FALSE}
LC <- rbind(LC0, LC100)
...
### Group 3 - Question A {#sec-A_3}

-   How are biodiversity indices changing in time and across the simulated scenario(s) on Plot 3?

Run the model as it is described in the previous chapters @sec-run, @sec-as and @sec-out to have at least 2 simulations completed: one with reference conditions and one scenario with some changes in the environment. Check if you have the output files in the output folder: `iLand_simulations/output/`. We show all of the solutions on the example of one reference and one alternative scenario output file. Open the `summerSchoolExercise.Rproj` project in R studio and start working there.

#### Read the tree output table {#sec-A_3.1}

Read in the *tree* output table from two outputs you have for your plot:

```{r ex47, eval=TRUE, message = FALSE, warning=FALSE}
...
```{r ex48, eval=TRUE, message = FALSE, warning=FALSE}
...
#### Visualize trees in year 0 and year 100  {#sec-A_3.2}

To explore a bit the simulation, plot the tree locations at the beginning and the end of the simulation For plotting we use the colors for the species, and the size of a circles to show dbh differences. Here we divided dbh with the value 20 just for visualization, not to have too huge circles shading each others completely, but still see the trees.

```{r ex49, eval=TRUE, message = FALSE, warning=FALSE}
tree0 <- tree |> dplyr::filter(year == 0)
g1 <-
    ggplot2::ggplot(tree0, ggplot2::aes(x = x, y = y, color = species)) +
    ggplot2::geom_point(size = tree0$dbh / 20, alpha = 0.7) +
...
#### Visualize some changes in time  {#sec-A_3.3}

Visualize some changes in time, for example the mean diameter of the trees

```{r ex50, eval=TRUE, message = FALSE, warning=FALSE}
sum.table <- tree |> dplyr::group_by(year, run) |>
    dplyr::summarise(
        N = dplyr::n(),
        MD = mean(dbh, na.rm = TRUE),
...
We can see that there is an initial increase in mean dbh, then a drop after 12/15 years. This is due to the growing regeneration layer that is produced by the seeds of the existing trees. In the initial year we do not have regeneration layer in the simulations (it is possible to put there, but we do not have it now). And until the small trees grow up to higher than 4 m they are not appearing in the individual *tree* output as they are handled in cohorts. After the stand canopy became closed, the mean DBH started to increase again until the end of the simulation period.

#### Species diversity {#sec-A_3.4}
...
```{r ex51, eval=TRUE, message = FALSE, warning=FALSE}
...
```{r ex52, eval=TRUE, message = FALSE, warning=FALSE}
...
```{r ex53, eval=TRUE, message = FALSE, warning=FALSE}
tlong2 <- tree2 |>
    dplyr::group_by(year, species) |> dplyr::summarise(N = dplyr::n()) |>
    tidyr::pivot_wider(
        id_cols = 'year',
...
```{r ex54, eval=TRUE, message = FALSE, warning=FALSE}
...
#### Spatial diversity {#sec-A_3.5}

We can calculate some additional biodiversity indices targeting spatial diversity using the *treespat* package developed by Francesco Chianucci (fchianucci\@gmail.com). You can find a short description of the package here: `documents/treespat_package.pdf`

You can install the package using devtools:

```{r ex55, eval=TRUE, message = FALSE, warning=FALSE}
devtools::install_gitlab('fchianucci/treespat')
```
...
```{r ex56, eval=TRUE, message = FALSE, warning=FALSE}
...
```{r ex57, eval=TRUE, message = FALSE, warning=FALSE}
...
The diameter differentiation increases with the increasing average difference in diameter between neighboring trees. The value 0 indicates that neighboring trees have equal diameters. The development shows that the index values decrease during the first circa 12 years, after which it steeply increases due to the occurrence of ingrowth and then the values are stabilized. The values of Shannon index differed between the scenarios only at the end of the simulation period, when the calculated Shannon index under the climate change scenario was slightly lower than under the reference climate. The mingling index is studying the neighboring trees regarding their species. In both development there is a decrease in the index after the occurrence of regeneration, because the number of trees increased four times. Using the climate change scenario the index shows greater decrease since the spruce share was substantially reduced (see Figures in Question B - Group 3, @sec-B_3.2).

### Group 3 - Question B {#sec-B_3}
...
#### Read the landscape output table {#sec-B_3.1}

Read in the *landscape* output table from two outputs you have for your plot:

```{r ex58, eval=TRUE, message = FALSE, warning=FALSE}
path1 <- "model/iLand_simulations/output/Output_plot3.sqlite"
path2 <-
    "model/iLand_simulations/output/Output_plot3_4deg_20percdrier.sqlite"
...
```{r ex59, eval=TRUE, message = FALSE, warning=FALSE}
...
#### Visualize changes in time {#sec-B_3.2}

We can see that the output is given for each year and each species in our 1ha area. Plot the living carbon (total_carbon_kg) in time, coloring by species. We give a unified species coloring in the beginning. Plot number of trees and mean diameter. Carbon and number of trees can be plotted in an additive way, but for mean dbh we do line plot per species.

```{r ex60, eval=TRUE, message = FALSE, warning=FALSE}
...
Beech profited from drier and warmer conditions at the expense of spruce. *Acer pseudoplatanus* and *Tilia cordata* also slightly increased their proportion in the living C stock and number of trees. The increase of the number of trees indicates the success of the regeneration under reference conditions, however this increase is much slower under the climate change scenario.
...
#### Assess the stored carbon amount {#sec-B_3.3}

Calculate the stored C amount in the initial year (year==0) and the last year (year==100)

```{r ex61, eval=TRUE, message = FALSE, warning=FALSE}
livingC0 <- data.frame(
    landscape |> dplyr::filter(year == 0) |>
        dplyr::group_by(run) |>
        dplyr::summarize(sum.livingC = sum(total_carbon_kg / 1000))
...
```{r ex62, eval=TRUE, message = FALSE, warning=FALSE}
species.livingC0 <-
    data.frame(
        landscape |> dplyr::filter(year == 0) |>
            dplyr::group_by(run, species) |>
...
#### Assess the species proportions {#sec-B_3.4}

Calculate the species proportions based on the stored C amount in the initial year (year==0) and the last year (year==100) For this we need the total C amount and the species-specific C amount.

```{r ex63, eval=TRUE, message = FALSE, warning=FALSE}
LC0 <- dplyr::left_join(species.livingC0, livingC0, by = "run")
LC0 <-
...
```{r ex64, eval=TRUE, message = FALSE, warning=FALSE}
LC <- rbind(LC0, LC100)
...
```{r ex65, eval=TRUE, message = FALSE, warning=FALSE}
...
```{r ex66, eval=TRUE, message = FALSE, warning=FALSE}
...
#### Visualize trees in year 0 and year 100  {#sec-A_4.2}

To explore a bit the simulation, plot the tree locations at the beginning and the end of the simulation For plotting we use the colors for the species, and the size of a circles to show dbh differences. Here we divided dbh with the value 20 just for visualization, not to have too huge circles shading each others completely, but still see the trees.

```{r ex67, eval=TRUE, message = FALSE, warning=FALSE}
tree0 <- tree |> dplyr::filter(year == 0)
g1 <-
    ggplot2::ggplot(tree0, ggplot2::aes(x = x, y = y, color = species)) +
    ggplot2::geom_point(size = tree0$dbh / 20, alpha = 0.7) +
...
#### Visualize some changes in time  {#sec-A_4.3}

Visualize some changes in time, for example the mean diameter of the trees

```{r ex68, eval=TRUE, message = FALSE, warning=FALSE}
sum.table <- tree |> dplyr::group_by(year, run) |>
    dplyr::summarise(
        N = dplyr::n(),
...
We can see that there is an initial increase in mean dbh, then a drop after 15/20 years followed by a less steep increase until the end of the simulation period. This is due to the growing regeneration layer that is produced by the seeds of the existing trees. In the initial year we do not have regeneration layer in the simulations (it is possible to put there, but we do not have it now). And until the small trees are less than 4 m tall, they do not appear in the individual *tree* output as they are handled in cohorts. When the canopy closes around the age 30/40, no more massive ingrowth occurs and mean DBH starts to grow again.

#### Species diversity {#sec-A_4.4}
...
```{r ex69, eval=TRUE, message = FALSE, warning=FALSE}
...
```{r ex70, eval=TRUE, message = FALSE, warning=FALSE}
...
```{r ex71, eval=TRUE, message = FALSE, warning=FALSE}
tlong2 <- tree2 |>
    dplyr::group_by(year, species) |> dplyr::summarise(N = dplyr::n()) |>
    tidyr::pivot_wider(
        id_cols = 'year',
...
```{r ex72, eval=TRUE, message = FALSE, warning=FALSE}
...
We see that the values of Shannon index are first stable and then tend to decrease, while the reduction under the *4deg_20percdrier* scenario is slightly greater than under the reference scenario due to the more pronounced dominance of beech (see Figures in Question B - Group 4, @sec-B_4.2).

#### Spatial diversity {#sec-A_4.5}

We can calculate some additional biodiversity indices targeting spatial diversity using the *treespat* package developed by Francesco Chianucci (fchianucci\@gmail.com). You can find a short description of the package here: `documents/treespat_package.pdf`

You can install the package using devtools:

```{r ex73, eval=TRUE, message = FALSE, warning=FALSE}
devtools::install_gitlab('fchianucci/treespat')
```
...
```{r ex74, eval=TRUE, message = FALSE, warning=FALSE}
# diameter differentiation:
# mingling
...
```{r ex75, eval=TRUE, message = FALSE, warning=FALSE}
...
The diameter differentiation increases with the increasing average difference in diameter between neighboring trees. The value 0 indicates that neighboring trees have equal diameters. The development shows that the index values decrease during the first 15/20 years, after which increase due to the occurrence of ingrowth and then the values are reduced again to their minima in 37/50 years, after which they again slightly increase. The values for the reference climate scenario are slightly lower than for the climate change scenario indicating slower growth. The mingling index is studying the neighboring trees regarding their species. No substantial difference in species mingling between scenarios was detected.

### Group 4 - Question B {#sec-B_4}
...
#### Read the landscape output table {#sec-B_4.1}

Read in the *landscape* output table from two outputs you have for your plot:

```{r ex76, eval=TRUE, message = FALSE, warning=FALSE}
path1 <- "model/iLand_simulations/output/Output_plot4.sqlite"
path2 <-
    "model/iLand_simulations/output/Output_plot4_4deg_20percdrier.sqlite"
...
```{r ex77, eval=TRUE, message = FALSE, warning=FALSE}
...
#### Visualize changes in time {#sec-B_4.2}

We can see that the output is given for each year and each species in our 1ha area. Plot the living carbon (total_carbon_kg) in time, coloring by species. We give a unified species coloring in the beginning. Plot number of trees and mean diameter. Carbon and number of trees can be plotted in an additive way, but for mean dbh we do line plot per species.

```{r ex78, eval=TRUE, message = FALSE, warning=FALSE}
...
At the end of the simulated 100 years, the total simulated carbon stock in living biomass under climate change scenario was slightly higher than under the reference climate. The proportion of beech has increased under both simulations. The number of trees increased in both cases after circa 25 years, but the increase was steeper under reference conditions. This increase was caused by the regeneration. However, at the end of the simulation period, the number of remaining trees was slightly higher under climate change scenario than the reference one, indicating much higher tree mortality.
...
#### Assess the stored carbon amount {#sec-B_4.3}

Calculate the stored C amount in the initial year (year==0) and the last year (year==100)

```{r ex79, eval=TRUE, message = FALSE, warning=FALSE}
livingC0 <- data.frame(
    landscape |> dplyr::filter(year == 0) |>
        dplyr::group_by(run) |>
        dplyr::summarize(sum.livingC = sum(total_carbon_kg / 1000))
...
```{r ex80, eval=TRUE, message = FALSE, warning=FALSE}
...
#### Assess the species proportions {#sec-B_4.4}

Calculate the species proportions based on the stored C amount in the initial year (year==0) and the last year (year==100) For this we need the total C amount and the species-specific C amount.

```{r ex81, eval=TRUE, message = FALSE, warning=FALSE}
LC0 <- dplyr::left_join(species.livingC0, livingC0, by = "run")
LC0 <-
    data.frame(LC0 |> dplyr::mutate(spec.prop = livingC / sum.livingC, year =
                                        0))
...
```{r ex82, eval=TRUE, message = FALSE, warning=FALSE}
LC <- rbind(LC0, LC100)
...
The data was collected from square sampling plots (2500 m2 each) in six forested Czech Republic regions. These regions are representative of the main bio-regions and elevation range of forests, considering their importance in territorial representation, forestry, and ecology. The inventory sampled for biodiversity variables such as the presence/absence of different species of birds, Tracheophyta, Bryophyte, fungi, lichens, and beetles.
...
-   `Bryophytes_rich` : Species richness across Bryophyte species
...
-   `dendrocoposMajor`: presence / absence of Great spotted woodpecker (*Dendrocopos major*)
-   `certhia`: presence / absence of Eurasian treecreeper (*Certhia familiaris*)
-   `bryophyteNumObs`: number of observed Bryophyte species
...
Bryophyte constitute an important and permanent component of the forest flora and diversity. They colonize various substrates, which are unsuitable for vascular plants, because of low light intensity or low nutrient level, such as deadwood, bark, rocks, and open soil. They provide shelter habitats, food, and nest material for many animals.

In forests, different ecological guilds of Bryophyte can be distinguished by the substrate on which they are growing, including terricolous, lignicolous, corticolous and saxicolous species that occur on soil, deadwood, bark of living trees and shrubs, or rocks, respectively. As diversity and quality of these substrates is affected by forest management, Bryophyte are suitable indicators for the effect of management on forest conditions. Especially typical woodland Bryophyte, which are strictly depending on forest conditions. It is interesting to better understand the relation of forest management effects on Bryophyte and some studies have already demonstrated their sensitivity to management practices.

### Species 2 - Great spotted woodpecker

The great spotted woodpecker (*Dendrocopos major*) is a medium-sized woodpecker with pied black and white plumage and a red patch on the lower belly. It is found in a wide variety of woodlands, broadleaf, coniferous or mixed forests. The great spotted woodpecker spends much of its time climbing trees. It is a generalist bird species.

### Species 3 - Eurasian treecreeper

The Eurasian treecreeper (*Certhia familiaris*) is a small passerine bird. It prefers mature trees, and in most of Europe, it tends to be found mainly in coniferous forest, especially spruce and fir.

## Models

### Generalized Linear Models (GLMs)

We propose to use a generalized linear model (GLM) to understand the abundance of different Bryophyte and birds species in the 99 plots. Count data often conform to a Poisson distribution; in this case, we have a count of the number of species recorded at each plot.
...
-   the `bag fraction`: The bag fraction provides information on which fraction [0-1] of the entire data should be drawn randomly to fit the new tree. This parameter includes a random probabilistic component, making each run model different, and is aimed at improving model accuracy, speed of model creation, and the reduction of overfitting (Friedman et al., 2008).
...
We highly recommend that you open the `summerSchoolExercise.Rproj` from the project folder that you now you have in your computer after following the instructions in @sec-projFolder.

We also highly recommend that you storage any Rscript that you create in the folder `code`.

### Find the data

If you followed the instructions in @sec-projFolder you should have the file `observations.csv` inside the folder `data`of your project file:
...
hist(
    observations$bryophyteNumObs,
    main = "",
    breaks = 10,
    col = rgb(0.3, 0.5, 1, 0.4) ,
    xlab = "Number of observed Bryophyte"
)
plot(
    y = observations$bryophyteNumObs,
...
Here you can see that the coefficient table produced by a GLM is very similar to a linear model. The intercept tells us the estimated value of the response variable when the continuous explanatory variables (here, just the Gini index) has a value of 0. We then also have coefficients describing the slope of the relationship with our continuous explanatory variables. We can see here that the number of Bryophyte species appears to show a positive relationship with the Gini index, which means that increasing structural diversity in tree diameter sizes has a positive relationship with the number of Bryophyte species in the plot.

We are also interested in understanding the relationship of the number of Bryophyte species and the tree species diversity; we can now try to add this variable into the model and see if it helps us to understand things. You can do that by adding the variable `ShannonIndexTreeSpp` to the model:

```{r fit glm struct12}
bryoModel2 <- glm(bryophyteNumObs ~ GiniDBH + ShannonIndexTreeSpp,
                  family = poisson,
                  data = observations)
...
Variability in forest structure (Gini index) explain 15% of the variation in Bryophyte species richness in this study. That is an ok explanatory power for a very simple model of a complex ecological system (many factors determine the species richness for Bryophyte and we are attempting to explain everything with just one variable).

#### Model Assumptions

For Poisson GLMs, there is one further assumption that we have not encountered before. If data follow a Poisson distribution, then the mean of the distribution is equal to the variance. Accordingly, a Poisson distribution is represented by just one parameter $\lambda$, which describes both the mean and the variance of the distribution.
...
We can get look whether a model is over-dispersed by inspecting the model summary as you did in @sec-C_1_fit. As a rule of thumb, if the response variable conforms to a true Poisson distribution, we expect the residual deviance to be approximately equal to the residual degrees of freedom. If the deviance is much greater than the degrees of freedom, this indicates over-dispersion. This is the case in our models (see outputs from running the code in @sec-C_1_fit).
...
We could start by only looking at how the forest structural diversity affects the number Bryophyte species that we have in a plot. You can do this by creating a GLM model which has `bryophyteNumObs` as response variable and `GiniDBH` as explanatory variable. For that you will use the function `glm` in R and use the `family = poisson`. You can see how to create and see this model here:
...
Here you can see that the coefficient table produced by a GLM is very similar to a linear model. The intercept tells us the estimated value of the response variable when the continuous explanatory variables (here just Gini index) have a value of 0. We then also have coefficients describing the slope of the relationship with our continuous explanatory variables. We can see here that the bryophyte numbers appears to show a positive relationship with Gini index, which means that increasing structural diversity in trees diameter sizes has a positive relationship with the number of Bryophyte species in the plot.

We are also interested in understanding the relationship of the number of observed Bryophyte species and forest management, we can now try create a new model considering this variable.

```{r fit glm struct22}
bryoModel2 <- glm(bryophyteNumObs ~ forestManagementType,
                  family = poisson,
                  data = observations)
...
The intercept here tells us the estimated value of the response variable when the reference groups in our grouping (categorical) variables (here for retention clear-cutting). We can see here that number of Bryophyte species appears to show a negative relationship with simple clearcutting, and appears to have a positive relationship with unmanaged and retention clear-cutting.
...
Variability in forest structure (Gini index) explains 15% of the variation in Bryophyte species richness in this study system. That is an ok explanatory power for a very simple model of a complex ecological system (many factors determine the species richness of Bryophyte and we are attempting to explain everything with one variable).
...
We can get look whether a model is over-dispersed by inspecting the model summary as you did in @sec-D_1_fit. As a rule of thumb, if the response variable conforms to a true Poisson distribution, we expect the residual deviance to be approximately equal to the residual degrees of freedom. If the deviance is much greater than the degrees of freedom, this indicates over-dispersion. This is the case in our models (see outputs from running the code in @sec-D_1_fit).
...
In these plots the outliers are highlighted in red (if they are any). To provide a visual aid in detecting deviations from uniformity in y-direction, the plot function calculates a quantile regression of the residuals, by default for the 0.25, 0.5 and 0.75 quantiles. As the residuals should be uniformly distributed for a correctly specified model, the theoretical expectations for these regressions are straight lines at 0.25, 0.5 and 0.75, which are displayed as dashed black lines on the plot. Some deviations from these expectations are to be expected by chance, however, even for a perfect model, especially if the sample size is small. The function therefore tests if deviation of the fitted quantile regression from the expectation is significant, using testQuantiles. If so, the significant quantile regression will be highlighted as red, and a warning will be displayed in the plot. You can find more information about this graphs in the [package instructions here](https://cran.r-project.org/web/packages/DHARMa/vignettes/DHARMa.html).
...
We can get look whether a model is over-dispersed by inspecting the model summary as you did in @sec-C_2_fit. As a rule of thumb, if the response variable conforms to a true Poisson distribution, we expect the residual deviance to be approximately equal to the residual degrees of freedom. If the deviance is much greater than the degrees of freedom, this indicates over-dispersion. This is the case in our models (see outputs from running the code in @sec-C_2_fit).
...
We are also interested in understanding the relationship of the number of observed bird species and forest management, we can now try to to create a new model with this variable.
...
The intercept here tells us the estimated value of the response variable when the reference groups in our grouping (categorical) variables (here for retention clear-cutting). We can see here that number of bird species appears to show a negative relationship with simple clearcutting, unmanaged and a positive relationship with retention clear-cutting.
...
Variability in forest structure (Gini index) explain 8% of the variation in bird species richness in this study system. That is an ok explanatory power for a very simple model of a complex ecological system (many factors determine the species richness and we are attempting to explain everything with one variable).
...
We can get look whether a model is over-dispersed by inspecting the model summary as you did in @sec-D_2_fit. As a rule of thumb, if the response variable conforms to a true Poisson distribution, we expect the residual deviance to be approximately equal to the residual degrees of freedom. If the deviance is much greater than the degrees of freedom, this indicates over-dispersion. This is the case in our models (see outputs from running the code in @sec-D_2_fit).
...
Unfortunately the amount of observations that we have in this dataset it is not enough to fit a BRT model for these variables. We are going to do an obviously wrong thing for the sake of being able to demonstrate how to fit a BRT model. In the next code you are going to repeat the same dataset multiple times:
...
# One variables is a character, we assign to factor instead:
...
Running a model such as that described above writes progress reports to the screen, makes a graph, and returns an object containing a number of components. The R console reports a brief model summary, all the model summary values are retained in the model object.
...
It seems that there is an increase in probability of finding a Great spotted woodpecker with higher forest densities, but the trend it is not very clear. We could also analyse the interaction effects, of density and for example the Gini index. The model predictions can be obtained for each pair of predictor variables, setting all other predictors to their means.

To plot this pairwise interactions we have to do:

```{r pairwise int, warning=FALSE, message=FALSE, fig.width = 9, fig.height = 9}
...
Unfortunately the amount of observations observations that we have in this dataset it is not enough to fit a BRT model for these variables. We are going to do an obviously wrong thing for the sake of being able to demonstrate how to fit a BRT model. In the next code you are going to repeat the same dataset multiple times:
...
# One variable are character, we assign to factor instead:
modelDataSel$forestManagementType <-
    as.factor(modelDataSel$forestManagementType)
...
```{r pairwise int6, warning=FALSE, message=FALSE, , fig.width = 9, fig.height = 9}
...
# One variable is a character, we assign to factor instead:
modelDataSel$forestManagementType <-
    as.factor(modelDataSel$forestManagementType)
...
Here we can see that the two variables with the highest influence in the response are `GiniDBH` and `latitude`.
...
In this partial dependence plots the predictions are on the scale of f(x). In this case, for the Bernoulli loss the returned value is on the log odds scale. You can see how this plot will look by plotting with the function from the package `gbm` and using the type "response". Since we are interesting in finding out if the forest management type has an impact in the presence of the Eurasian treecreeper affected we can have a look to the plot density variable `forestManagementType` :
...
In this partial dependence plots the predictions are on the scale of f(x). In this case, for the Bernoulli loss the returned value is on the log odds scale. You can see how this plot will look by plotting with the function from the package `gbm` and using the type "response". Since we are interestedg in finding out if the forest density has an impact in the presence of the Eurasian treecreeper affected we can have a look to the plot density variable `volAllha` :
...
```{r pairwise int8, warning=FALSE, message=FALSE, fig.width = 9, fig.height = 9}
...
# References and further reading {#sec-ref}
...
Elith, J., J. R. Leathwick, and T. Hastie. 2008. "A Working Guide to Boosted RegressionTrees."Journal of Animal Ecology77 (4): 802--13.https://doi.org/10.1111/j.1365-2656.2008.01390.x.

Friedman, Jerome, Trevor Hastie, and Robert Tibshirani. n.d. "ADDITIVE LOGISTICREGRESSION: A STATISTICAL VIEW OF BOOSTING."
...
# Appendix - Initialize trees for the model based on inventory data {#sec-app}

## Inventory data

We had inventory data prior the summer school. These plots were established in 2007 for monitoring of urban forest Rožnik. Those four plots were already remeasured by NFI teams (for yearly training) at the beginning of April 2023. The radius of the plots is 13.8m and trees dbh\>=10cm were measured.

The original number of these plots are 3, 4, 8, 9 but in this exercise this are respectively: plot 1, plot 2, plot 3 and plot 4. 
...
### Read in the inventory data to R visualize it. Read also a table with species latin names and short names in iLand.

```{r app1, eval=TRUE, message = FALSE, warning=FALSE}


a <-
    openxlsx::read.xlsx(here::here("model/iLand_simulations/init/trees/testPlot_data_Roznik.xlsx"))
...
### Calculate some statistics for populating remaining areas

Not all records have height, so we calculate H:D ratios for each tree that has.

```{r app2, eval=TRUE, message = FALSE, warning=FALSE}
...
### Generate trees for each plot

-   Using the number of trees/ha
-   Generate dbh of individual trees based on a normal distribution using mean dbh and stdev
...
```{r app3, eval=TRUE, message = FALSE, warning=FALSE}
...
### Lets remove the trees in the middle, and put back the real trees from the inventory

```{r app4, eval=TRUE, message = FALSE, warning=FALSE}
...
### Give height and plot final layout

```{r app5, eval=TRUE, message = FALSE, warning=FALSE}
